class Sensor {
     ...
     #getReading(ray,roadBorders,traffic){
          ...
          if(touches.length == 0) return null;
               ...
               return touches.find(event => event.offset == minOffset);
          }
     }
     #castRays(){
          this.rays = [];
     ...
     draw(zim){
          for(let i = 0;i < this.rayCount;i++){
               ...
               zim.beginPath();
               zim.lineWidth = 2;
               zim.strokeStyle = 'yellow';
               zim.moveTo(this.rays[i][0].x,this.rays[i][0].y);
               zim.lineTo(end.x,end.y);
               zim.stroke();
               zim.beginPath();
               zim.lineWidth = 2;
               zim.strokeStyle = 'black';
               zim.moveTo(this.rays[i][1].x,this.rays[i][1].y);
               zim.lineTo(end.x,end.y);
               zim.stroke();
          }
     }
